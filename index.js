...
`).join("");Y.insertAdjacentHTML("beforeend",o)}function D(){L.classList.add("loader")}function x(){L.classList.remove("loader")}function H(e,t,r){Math.ceil(t/r)>e?R():k()}function R(){v.classList.remove("hidden")}function k(){v.classList.add("hidden")}function _(){let t=document.querySelector(".art-item").getBoundingClientRect();window.scrollBy({top:t.height*2,behavior:"smooth"})}document.querySelector(".art-btn-loadMore").addEventListener("click",I);document.getElementById("loader");let m=1;const g=8,w=async()=>{try{D();const e=await O(m,g);x(),H(m,e.totalArtists,g),C(e.artists),document.querySelectorAll(".art-btn-learnMore").forEach(r=>r.addEventListener("click",T))}catch(e){console.error("Помилка в getListArtist:",e)}m>1&&_()};function I(e){e.preventDefault(),++m,k(),w()}w();const G="https://sound-wave.b.goit.study/api";async function W(){try{const t=(await h.get(`${G}/feedbacks`)).data.data;return Array.isArray(t)?t:[]}catch(e){return console.error("Ошибка при загрузке отзывов:",e),[]}}function K({rating:e,descr:t,name:r}){return`
