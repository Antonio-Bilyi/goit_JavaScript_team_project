...
`).join("");Y.insertAdjacentHTML("beforeend",o)}function C(){k.classList.add("loader")}function x(){k.classList.remove("loader")}function H(t,e,r){Math.ceil(e/r)>t?R():$()}function R(){L.classList.remove("hidden")}function $(){L.classList.add("hidden")}function G(){let e=document.querySelector(".art-item").getBoundingClientRect();window.scrollBy({top:e.height*2,behavior:"smooth"})}document.querySelector(".art-btn-loadMore").addEventListener("click",I);document.getElementById("loader");let m=1;const g=8,w=async()=>{try{C();const t=await O(m,g);x(),H(m,t.totalArtists,g),_(t.artists),document.querySelectorAll(".art-btn-learnMore").forEach(r=>r.addEventListener("click",j))}catch(t){console.error("Помилка в getListArtist:",t)}m>1&&G()};function I(t){t.preventDefault(),++m,$(),w()}w();const K="https://sound-wave.b.goit.study/api";async function W(){try{const e=(await y.get(`${K}/feedbacks`)).data.data;return Array.isArray(e)?e:[]}catch(t){return console.error("Ошибка при загрузке отзывов:",t),[]}}function J({rating:t,descr:e,name:r}){return`
